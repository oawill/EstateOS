...
import { countOpenHousekeepingTasks } from "./housekeeping";
...
export async function getShortletDashboardSummary(estateId: string, now = new Date()) {
  ...
  const [
    ...
    countOpenHousekeepingTasks(estateId),
    prisma.maintenanceTicket.count({
      where: { estateId, shortletUnitId: { not: null }, status: { notIn: ["RESOLVED", "CLOSED"] } },
    }),
  ]);
  ...
  const occupancyRate = totalUnits > 0 ? Math.round((currentGuests / totalUnits) * 100) : 0;
