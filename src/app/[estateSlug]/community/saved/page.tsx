...
import { listSavedPosts } from "@/server/modules/community/posts";
import { listSavedListings } from "@/server/modules/community/classifieds";
import { getOrCreateCommunitySettings } from "@/server/modules/community/settings";
import { FeedList } from "../FeedList";

export default async function SavedPage({ params }: { params: Promise<{ estateSlug: string }> }) {
  ...
  const { residentId, posts, listings, defaultDisplayNamePreference } = await guardPage(async () => {
    ...
    const [posts, listings, settings] = await Promise.all([
      listSavedPosts(membership.estateId, resident.id),
      listSavedListings(membership.estateId, resident.id),
  ...
  return (
    <div className="space-y-6">
      <section className="space-y-2">
        <h2 className="text-sm font-medium text-foreground-muted">Saved listings</h2>
        {listings.length === 0 ? (
          <p className="text-sm text-foreground-muted">No saved listings yet.</p>
        ) : (
            ...
            <Link key={listing.id} href={`/${estateSlug}/community/classifieds/${listing.id}`}>
              <Card className="hover:border-primary/40">
                <div className="flex items-center justify-between">
                  ...
                  <Badge tone={listing.status === "ACTIVE" ? "info" : "neutral"}>{listing.status}</Badge>
                </div>
              </Card>
            </Link>
          ))
      ...
      <section className="space-y-2">
        <h2 className="text-sm font-medium text-foreground-muted">Saved posts</h2>
        <FeedList
