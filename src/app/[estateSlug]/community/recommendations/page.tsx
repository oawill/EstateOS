...
export default async function RecommendationsPage({ params }: { params: Promise<{ estateSlug: string }> }) {
  ...
  const { residentId, items, defaultDisplayNamePreference } = await guardPage(async () => {
    ...
    const [items, settings] = await Promise.all([
      listFeed(membership.estateId, { postType: "RECOMMENDATION" }),
      getOrCreateCommunitySettings(membership.estateId),
    ]);
    return { residentId: resident.id, items, defaultDisplayNamePreference: settings.defaultDisplayNamePreference };
  });
