...
export default async function NewMeterPage({ params }: { params: Promise<{ estateSlug: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-lg space-y-6">
      <h1 className="text-xl font-semibold">Add meter</h1>
      <Card>
        <NewMeterForm
          estateSlug={estateSlug}
          units={units.map((u) => ({ id: u.id, label: u.label, propertyAddressLabel: u.property.addressLabel }))}
        />
      </Card>
    </div>
  );
